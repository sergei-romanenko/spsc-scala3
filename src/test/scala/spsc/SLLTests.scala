...
class SLLTests extends AnyFunSuite {
  ...
  test(testName = "Term.toString 102 StrLet") {
    assertToStr(Let(Var("y"), List(("x", Var("y")))), "let x=y in y")
    assertToStr(
      Let(Var("x"), List(("x", Var("a")), ("y", Var("b")))),
      "let x=a,y=b in x"
    )
  }
  ...
  test(testName = "Rule.toString 103 StrRule") {
    assert(
      FRule("f", List("x", "y"), Var("y")).toString
        === "f(x,y)=y;"
    )
    assert(
      GRule("g", Pat("C", List("x")), List("y"), Var("y")).toString
        === "g(C(x),y)=y;"
    )
    assert(
      GRule("g", Pat("C", Nil), List("y"), Var("y")).toString
        === "g(C,y)=y;"
    )
    assert(
      GRule("g", Pat("C", Nil), Nil, Ctr("C", Nil)).toString
        === "g(C)=C;"
    )
  }
  ...
  test(testName = "Program.toString 104 StrProgram") {
    assert(
      List(FRule("f", Nil, Ctr("A", Nil)), FRule("f1", Nil, Ctr("A1", Nil)))
        .mkString("")
        === "f()=A;f1()=A1;"
    )
    assert(
      List(
        ...
        GRule("g2", Pat("C", List("x")), Nil, Ctr("A", Nil))
      ).mkString("")
