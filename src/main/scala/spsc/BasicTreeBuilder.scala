...
class BasicTreeBuilder(task: Task):
  ...
  val getGs: Map[Name, List[GRule]] =
    task.rules.foldRight(Map[Name, List[GRule]]()):
      case (r: GRule, m) =>
        m + (r.name -> (r :: m.getOrElse(r.name, Nil)))
      case (_, m) => m
  ...
  def driveTerm(term: Term): List[Branch] = (term: @unchecked) match
    case Ctr(name, args) =>
      args.map((_, None))
    case FCall(name, args) =>
      val f = getF(name)
      val subst = Map(f.params.zip(args)*)
      List((applySubst(subst)(f.term), None))
    case GCall(name, arg0 :: args) =>
      arg0 match
        case Ctr(cname, cargs) =>
          val g = getG(name, cname)
          val subst = Map(g.allParams.zip(cargs ::: args)*)
        ...
        case v: Var =>
          for g <- getGs(name) yield
            ...
            val subst = Map(g.allParams.zip(cargs1 ::: args1)*)
            (applySubst(subst)(g.term), c)
        case _ =>
          val bs = driveTerm(arg0)
          for (t, c) <- bs
          yield (GCall(name, t :: args.map(applyContr(c))), c)
